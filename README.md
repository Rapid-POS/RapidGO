...
````
The pound signs showing in GitHub means you are copying rendered text, not raw markdown. 

The only way to copy raw markdown from Claude is to use the **copy button on a code block**. Here it is in a code block — use the **copy button in the top right corner of the box below**, then paste into GitHub:
...
> **Warning:** Your environment must meet Rapid POS connectivity and network requirements before installation. Troubleshooting or follow-up work resulting from unmet requirements will be billed at standard T&M rates. If your environment does not meet requirements, consult your Care Team Lead (vCIO).
